...
static int fb_lfs_fbarrier(fb_fdesc_t *);
static int fb_lfs_fdatabarrier(fb_fdesc_t *);
...
static fsplug_func_t fb_lfs_funcs =
{
	...
	fb_lfs_fsync,		/* fsync */
	fb_lfs_fbarrier,	/* fbarrier */
	fb_lfs_fdatabarrier,	/* fdatabarrier */
	fb_lfs_stat,		/* stat */
	fb_lfs_fstat,		/* fstat */
...
/* 
 * [UFS] fbarrier and fdatabarrier is added.
 * Does fbarrier|fdatabarrier of a file. Returns with ops return info.
 */
#define fbarrier(fd) 	 syscall(327,(fd))
#define fdatabarrier(fd) syscall(328,(fd))

static int 
fb_lfs_fbarrier(fb_fdesc_t *fd)
{
	return (fbarrier(fd->fd_num));
}

static int
fb_lfs_fdatabarrier(fb_fdesc_t *fd)
{
	return (fdatabarrier(fd->fd_num));
}
