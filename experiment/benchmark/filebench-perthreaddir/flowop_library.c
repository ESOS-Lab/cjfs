...
static int flowoplib_fbarrier(threadflow_t *, flowop_t *flowop);
static int flowoplib_fdatabarrier(threadflow_t *, flowop_t *flowop);
...
static flowop_proto_t flowoplib_funcs[] = {
	...
	{FLOW_TYPE_IO, 0, "fbarrier", flowop_init_generic,
	flowoplib_fbarrier, flowop_destruct_generic},
	{FLOW_TYPE_IO, 0, "fdatabarrier", flowop_init_generic,
	flowoplib_fdatabarrier, flowop_destruct_generic},
...
#define MON_CTIME_ADD(idx) \
do {\
	idx = atomic_fetch_add_explicit(&filebench_shm->shm_index,	\
										1, memory_order_relaxed);		\
	if (idx >= MON_MAX_INDEX) {idx = -1; break;}						\
	filebench_shm->shm_ctime[idx] = gethrtime();					\
} while (0)

#define MON_DELAY_ADD(idx) \
do { \
	unsigned long long stime; 					\
	if (idx < 0) break;							\
	stime = filebench_shm->shm_ctime[idx];		\
	filebench_shm->shm_delay[idx] = gethrtime() - stime; \
} while (0)
/*
...
flowoplib_pickfile(filesetentry_t **filep, flowop_t *flowop, int flags, int tid)
{
	...
	if ((*filep = fileset_pick_perthread(fileset, FILESET_PICKFILE | flags,
	    0, fileindex, tid)) == NULL) {
...
flowoplib_openfile_common(threadflow_t *threadflow, flowop_t *flowop, int fd)
{
	...
	if ((err = flowoplib_pickfile(&file, flowop,
	    FILESET_PICKEXISTS, threadflow->tf_utid)) != FILEBENCH_OK) {
		filebench_log(LOG_DEBUG_SCRIPT,
	...
	flowop_beginop(threadflow, flowop);
	err = fileset_openfile_perthread(&threadflow->tf_fd[fd], flowop->fo_fileset,
	    file, openflag, 0666, flowoplib_fileattrs(flowop), threadflow->tf_utid);
	flowop_endop(threadflow, flowop, 0);
...
flowoplib_createfile(threadflow_t *threadflow, flowop_t *flowop)
{
	...
	if ((err = flowoplib_pickfile(&file, flowop,
	    FILESET_PICKNOEXIST, threadflow->tf_utid)) != FILEBENCH_OK) {
		filebench_log(LOG_DEBUG_SCRIPT,
		    "flowop %s failed to pick file from fileset %s",
		    flowop->fo_name,
		    avd_get_str(flowop->fo_fileset->fs_name));
...
flowoplib_deletefile(threadflow_t *threadflow, flowop_t *flowop)
{
	...
	fileset_unbusy_perthread(file, TRUE, FALSE, -file->fse_open_cnt, threadflow->tf_utid);
...
/*
 * [UFS] fbarrier & fdatabarrier is added for flow operation library.
 */
static int
flowoplib_fbarrier(threadflow_t *threadflow, flowop_t *flowop)
{
	filesetentry_t *file;
	int fd;
	...
	if (threadflow->tf_fd[fd].fd_ptr == NULL) {
		filebench_log(LOG_ERROR,
		    "flowop %s attempted to fbarrier a closed fd %d",
		    flowop->fo_name, fd);
		return (FILEBENCH_ERROR);
	}
	...
	if ((file == NULL) ||
	    (file->fse_fileset->fs_attrs & FILESET_IS_RAW_DEV)) {
		filebench_log(LOG_ERROR,
		    "flowop %s attempted to a fbarrier a RAW device",
		    flowop->fo_name);
		return (FILEBENCH_ERROR);
	}
...
#endif
	flowop_beginop(threadflow, flowop);
	(void) FB_FBARRIER(&threadflow->tf_fd[fd]);
	flowop_endop(threadflow, flowop, 0);
#ifdef MON_FBARRIER
	MON_DELAY_ADD(index);
#endif 

	filebench_log(LOG_DEBUG_SCRIPT, "fbarrier file %s", file->fse_path);

	return (FILEBENCH_OK);
}

static int
flowoplib_fdatabarrier(threadflow_t *threadflow, flowop_t *flowop)
{
	filesetentry_t *file;
	int fd;
	...
	if (threadflow->tf_fd[fd].fd_ptr == NULL) {
		filebench_log(LOG_ERROR,
		    "flowop %s attempted to fdatabarrier a closed fd %d",
		    flowop->fo_name, fd);
		return (FILEBENCH_ERROR);
	}
	...
	if ((file == NULL) ||
	    (file->fse_fileset->fs_attrs & FILESET_IS_RAW_DEV)) {
		filebench_log(LOG_ERROR,
		    "flowop %s attempted to a fdatabarrier a RAW device",
		    flowop->fo_name);
		return (FILEBENCH_ERROR);
	}
	...
	/* Measure time to fsync */
	flowop_beginop(threadflow, flowop);
	(void) FB_FDATABARRIER(&threadflow->tf_fd[fd]);
	flowop_endop(threadflow, flowop, 0);

	filebench_log(LOG_DEBUG_SCRIPT, "fdatabarrier file %s", file->fse_path);
...
flowoplib_closefile(threadflow_t *threadflow, flowop_t *flowop)
{
	...
	fileset_unbusy_perthread(file, FALSE, FALSE, -1, threadflow->tf_utid);
