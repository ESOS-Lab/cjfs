...
static void
dh_fileset_alloc_leafdir(filesetentry_t *entry, int index)
{
	fileset_t *fileset;
        char path[MAXPATHLEN];
        char *pathtmp;
...
static void 
dh_fileset_alloc_file(filesetentry_t *entry, int index)
{
	fileset_t *fileset;
	char path[MAXPATHLEN];
	char *buf;
	...
	FB_OPEN(&fdesc, path, O_RDWR | O_CREAT, 0644) ;
		  
	buf = (char *)malloc(FILE_ALLOC_BLOCK);
	...
	/* unbusy the allocated entry */
	fileset_unbusy_perthread(entry, TRUE, TRUE, 0, index);
}

static void 
dh_filesetentry_create(fileset_t *fileset, filesetentry_t **retentry, 
			filesetentry_t *parent, int index)
{
	filesetentry_t *entry;
	char tmpname[16];
	...
	(void) snprintf(tmpname, sizeof (tmpname), "%08d", index+1);
	if ((entry->fse_path = (char *)ipc_pathalloc(tmpname)) == NULL) {
		filebench_log(LOG_ERROR,
...
static int
fileset_populate_perthreaddir(fileset_t *fileset){
	filesetentry_t *temp, *ptdir, *file;
	int i, j;

	temp = (filesetentry_t *)ipc_malloc(FILEBENCH_FILESETENTRY);

	for (i = 0; i < MAX_THREAD; i++) {
		dh_filesetentry_create(fileset, &ptdir, temp, i);

		ptdir->fse_flags = FSE_TYPE_LEAFDIR | FSE_EXISTS;
		dh_fileset_alloc_leafdir(ptdir, i);

		for (j = 0; j < 100; j++) {
			dh_filesetentry_create(fileset, &file, ptdir, j);

			file->fse_flags = FSE_TYPE_FILE | FSE_FREE;
			avl_add(&fileset->fs_perthread[i].fs_free_files, file);
			dh_fileset_alloc_file(file, i);

			if (fileset->fs_filelist == NULL) {
				fileset->fs_filelist = file;
				file->fse_nextoftype = NULL;
			} else {
				file->fse_nextoftype = fileset->fs_filelist;
				fileset->fs_filelist = file;
			}
		}
	}
...
fileset_populate(fileset_t *fileset)
{
	...
	int i;
	for (i=0; i < MAX_THREAD; i++) {
		avl_create(&(fileset->fs_perthread[i].fs_free_files), fileset_entry_compare,
		    sizeof (filesetentry_t), FSE_OFFSETOF(fse_link));
		avl_create(&(fileset->fs_perthread[i].fs_noex_files), fileset_entry_compare,
		    sizeof (filesetentry_t), FSE_OFFSETOF(fse_link));
		avl_create(&(fileset->fs_perthread[i].fs_exist_files), fileset_entry_compare,
		    sizeof (filesetentry_t), FSE_OFFSETOF(fse_link));
	}
...
fileset_createsets()
{
	...
	while (list) {
		...
		fileset_populate_perthreaddir(list);
...
int
fileset_openfile_perthread(fb_fdesc_t *fdesc, fileset_t *fileset,
    filesetentry_t *entry, int flag, int filemode, int attrs, int threadid)
{
	char path[MAXPATHLEN];
	char dir[MAXPATHLEN];
	char *pathtmp;
	...
	if (FB_OPEN(fdesc, path, flag | open_attrs, filemode)
		...
		fileset_unbusy_perthread(entry, FALSE, FALSE, 0, threadid);
		return (FILEBENCH_ERROR);
	}
	...
	if (flag & O_CREAT)
		fileset_unbusy_perthread(entry, TRUE, TRUE, 1, threadid);
	else
		fileset_unbusy_perthread(entry, FALSE, FALSE, 1, threadid);

	return (FILEBENCH_OK);
}

filesetentry_t *
fileset_pick_perthread (fileset_t *fileset2, int flags, 
	int tid, int index, int threadid)
{
	filesetentry_t *entry = NULL;
	filesetentry_t *start_point;
	avl_tree_t *atp = NULL;
	fbint_t max_entries = 0;
	ptfileset_t *fileset = &fileset2->fs_perthread[threadid];
	...
	if (flags & FILESET_PICKUNIQUE) {
		...
		case FILESET_PICKFILE:
			if (flags & FILESET_PICKNOEXIST) {
				entry = fileset_find_entry(atp,
				    fileset2->fs_file_nerotor);
				fileset2->fs_file_nerotor =
				    entry->fse_index + 1;
			} else {
				entry = fileset_find_entry(atp,
				    fileset2->fs_file_exrotor[tid]);
				fileset2->fs_file_exrotor[tid] =
				    entry->fse_index + 1;
			}
			break;
...
void 
fileset_unbusy_perthread(filesetentry_t *entry, int update_exist,
    int new_exist_val, int open_cnt_incr, int threadid)
{
	fileset_t *fileset2 = NULL;
	ptfileset_t *fileset = NULL;

	if (entry) {
		fileset2 = entry->fse_fileset;
		fileset = &fileset2->fs_perthread[threadid];
	}
